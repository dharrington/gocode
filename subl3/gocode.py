...
def hint_and_subj(cls, name, type, insertArgs):
    subj = name
    if cls == "func":
        hint = cls + " " + name
...
class Gocode(sublime_plugin.EventListener):
    def on_query_completions(self, view, prefix, locations):
        ...
        cloc = "c{0}".format(loc)
        locline = view.line(loc)
        paren_index = view.substr(sublime.Region(loc, locline.b)).find("(")
        has_paren = paren_index >= 0 and paren_index < 3
        gocode = subprocess.Popen(["gocode", "-f=csv", "-filtersuggestions=0", "autocomplete", filename, cloc],
            stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        try:
            result = gocode.communicate(src.encode(), timeout=GOCODE_TIMEOUT)
        except:
            gocode.kill()
            print("gocode timeout")
            return
        out = result[0].decode()
        result = []
        for line in filter(bool, out.split("\n")):
            cls,name,type = line.split(",,")
            hint, subj = hint_and_subj(cls, name, type, not has_paren)
...
class GocodeGotoDefinition(sublime_plugin.TextCommand):
    def run(self, edit):
        ident,call=lookup(self.view, self.view.sel()[0])
...
def show_type_info(res, showdoc=True, showpos=True, dim=False):
    ...
    info = '<div style="font-size:12; padding:0px; margin:0px">'
    info += '<tt>{}</tt><br>'.format(desc)
...
class GocodePopupOnHover(sublime_plugin.EventListener):
    def on_hover(self, view, point, hover_zone):
        if not INFO_ON_HOVER: return
        if hover_zone != sublime.HOVER_TEXT: return
        ident,call=lookup(view, point)
        if ident is None:
            if call is None:
                return
            else:
        ...
        view.show_popup(info, location=point, max_width=1000, on_navigate=self.on_navigate)
...
class GocodeErrors(sublime_plugin.EventListener):
    ...
    def clear(self, view):
        ...
        view.erase_phantoms("gocodeerrorphantoms")
    ...
    def update_errors(self, view, show_all):
        ...
        gocode = subprocess.Popen(["gocode", "reporterrors", filename],
            stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        try:
            result = gocode.communicate(src.encode(), timeout=3)
        except:
            gocode.kill()
            return
        out = result[0].decode()
        ...
        for line in out.split('\n'):
            if line.startswith("Error:"):
                ...
                if show_all or not any([(e in line) for e in QUIET_ERRORS]):
                    ...
                    if SHOW_PHANTOMS:
                        view.add_phantom("gocodeerrorphantoms", view.word(pt), ' '.join(parts[3:]), sublime.LAYOUT_INLINE)
    ...
    def on_post_save_async(self, view):
        if not self.gosrc: return
        self.update_errors_later(view)
    def update_errors_later(self, view):
        # If not called for 1 second, call update_errors.
        if self.waiting == 0:
            def cb():
                if self.waiting==1:
                    self.waiting = 0
